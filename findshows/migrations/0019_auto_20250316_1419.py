...
def create_other_static_test_data(apps, schema_editor):
    ...
    default_creator_u_p = UserProfile.objects.create(
        ...
        weekly_email=False,
        given_artist_access_datetime=datetime.datetime(2025, 3, 21, 22, 43, 6, 633352, tzinfo=datetime.timezone.utc)
    )
    default_creator_u_p.given_artist_access_by=default_creator_u_p
    default_creator_u_p.save()

    local_artist_u_p = UserProfile.objects.create(
        user=User.objects.get(username="LOCAL_ARTIST"),
        weekly_email=False,
        given_artist_access_by=default_creator_u_p,
        given_artist_access_datetime=datetime.datetime(2025, 3, 21, 22, 43, 6, 633352, tzinfo=datetime.timezone.utc)
    )
